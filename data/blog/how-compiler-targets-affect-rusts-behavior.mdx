...
_Credits: This blog was inspired by a lesson given by [Daniel Cumming](https://x.com/danielkcumming)._
...
_Figure 3: Visual representation of the stack layout before the loop runs — a 5-byte `buffer` followed by
...
| ------ | ------------------------------- | ------------------------------ | ------------------- | -------------------------------- |
| –      | [ 00, 00, 00, 00, 00 ]          | –                              | –                   | [ D5, DD, 00, 00 ]               |
| 0–4    | [ 00, 01, 02, 03, 04 ]          | Writes to `buffer`             | 00 to 04            | [ D5, DD, 00, 00 ]               |
| 5–7    | +5 to +7                        | Writes to padding              | 05 to 07            | [ D5, DD, 00, 00 ]               |
...
2. Loop iterations 0–4 write the bytes `0x00` through `0x04` into the five `buffer` slots. These writes stay
   inside the buffer and do not touch `not_in_buffer`.

3. Iterations 5–7 write into the padding area between `buffer` and `not_in_buffer`. Because of the platform's
...
- Before: `[D5, DD, 00, 00]` → `0x00_00_DD_D5 = 56789`
- After: `[08, DD, 00, 00]` → `0x00_00_DD_08 = 56584`
...
2. Loop iterations 0–4 write `0x00`..`0x04` into the five `buffer` slots — still inside the buffer.
...
- Before: `[D5, DD, 00, 00]` → `0x00_00_DD_D5 = 56789`
- After: `[05, DD, 00, 00]` → `0x00_00_DD_05 = 56581`

5. Numeric arithmetic: the LSB changed from 0xD5 (213) to 0x05 (5). The integer decreases by 208 (213 − 5), so
   56789 − 208 = 56581.
